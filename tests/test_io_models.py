...
def test_read_instance_and_evaluate_closed_routes_by_default(tmp_path):
    ...
    assert solution.route_lengths(instance) == [16, 18]
    assert solution.evaluate(instance).as_tuple() == (18, 34, 2)


def test_evaluate_open_routes_when_requested(tmp_path):
    input_path = tmp_path / "tiny.txt"
    input_path.write_text(
        "\n".join(
            [
    ...
    instance = read_instance(input_path)
    solution = Solution([[0, 1, 3], [0, 2]], include_return_to_depot=False)
