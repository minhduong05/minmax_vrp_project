...
def route_length(route, d):
    ...
    if not route:
        return 0
    return sum(d[route[i]][route[i + 1]] for i in range(len(route) - 1)) + d[route[-1]][0]
...
def insertion_delta(route, point, position, d):
    ...
    next_node = route[position] if position < len(route) else 0
...
def remove_delta(route, position, d):
    ...
    prev = route[position - 1]
    cur = route[position]
    next_node = route[position + 1] if position < len(route) - 1 else 0
...
def two_opt_delta(route, startPos, endPos, d):
    ...
    next_node = route[endPos + 1] if endPos < len(route) - 1 else 0
    return costNew + d[s][next_node] - costOld - d[e][next_node]
